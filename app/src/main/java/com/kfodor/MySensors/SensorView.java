...
public class SensorView extends FragmentActivity implements
	...
	public void onSensorChanged(SensorEvent event) {
		...
		if (event.sensor.getType() == this.si.getSensor().getType()) {
			...
			if (settings.getLogFile().isEmpty() == false){
				writeSensorDataToFile(event,System.currentTimeMillis());
			}
		}
	}
	...
	// Simple write member function to dump sensor data
	private void writeSensorDataToFile(SensorEvent event, long timestamp) {
		// Write some info to the log about this sensor
		String text=String.valueOf(timestamp - log_start_time);
		double mag=0;
		...
		if(event.values.length > 1) {
			...
			text += "," + String.valueOf(mag);
		}
		text += "\n";

		try {
			if (datastream != null && datastream.getFD().valid()) {
	...
	private void StartLogFile(){
		File log_dir = Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOWNLOADS);
		String log_filename = settings.getLogFile();
		if(log_filename.isEmpty()){
			//Time now = new android.text.format.Time();
			Time now = new Time();
			now.setToNow();
			String timestamp = now.format("%Y%m%d-%H%M%S");
			log_filename=String.format("%s/sensorlog_%s.txt",log_dir.getPath(),timestamp);
			settings.setLogFile(log_filename);
		}
		if(!log_filename.isEmpty()) {
			try {
				if(datastream != null){
				...
				datastream = new FileOutputStream(log_filename,true);

				String sensor_description = String.format("# Sensor Type: %s\n# hashCode:%d\n# %s\n",
						SensorInterface.getType(si.getSensor().getType()), si
								.getSensor().hashCode(), si.getSensor().toString());

				log_start_time = System.currentTimeMillis();
				String text=sensor_description+"# time(ms)";
				for(int i = 0; i<si.getNumLabels(); i++){
					text+=String.format(",%s(%s)",si.getLabel(i),si.getUnits());
				}
				if(si.getNumLabels()>1){
					text+= String.format(",rms(%s)",si.getUnits());
				}
				text+="\n";
				datastream.write(text.getBytes());
				...
				Log.e(TAG,String.format("Error opening file: %s\n",log_filename));
			}
		}
		Log.d(TAG, String.format("Logging to file: %s\n",log_filename));
	}
	...
	private void StopLogFile(){
		...
		try {
			if (datastream != null && datastream.getFD().valid()) {
				...
				String notification_text = String.format("Saved to: %s",settings.getLogFile());
				// Post notification
				Toast.makeText(getApplicationContext(), notification_text,
						Toast.LENGTH_SHORT).show();
			}
